...
pip install psycopg2-binary -t "%LAMBDA_DIR%\query_sales\package" -q

REM Copiar shared
xcopy "%LAMBDA_DIR%\shared" "%LAMBDA_DIR%\query_sales\package\shared\" /E /I /Q /Y

REM Copiar archivos del handler (solo .py, no tests ni __pycache__)
...
pip install psycopg2-binary -t "%LAMBDA_DIR%\query_inventory\package" -q

xcopy "%LAMBDA_DIR%\shared" "%LAMBDA_DIR%\query_inventory\package\shared\" /E /I /Q /Y
...
pip install psycopg2-binary -t "%LAMBDA_DIR%\analyze_discounts\package" -q

xcopy "%LAMBDA_DIR%\shared" "%LAMBDA_DIR%\analyze_discounts\package\shared\" /E /I /Q /Y
...
REM Usar ruta corta sin espacios para los ZIPs
